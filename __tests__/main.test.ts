...
vi.mock('@actions/github', () => ({
  getOctokit: vi.fn().mockReturnValue({ graphql: vi.fn(), rest: { issues: { createComment: vi.fn() } } }), 
  context: {
...
beforeEach(async () => {
  ...
  MOCK_CREATE_COMMENT.mockResolvedValue({ data: { id: 100, html_url: 'new_comment_url' } } as any);
  MOCK_UPDATE_COMMENT.mockResolvedValue({ data: { id: 101, html_url: 'updated_comment_url' } }as any);
...
describe('Initial Checks', () => { 
  test('should log info and return early if pullRequestNumber is invalid (e.g. NaN)', async () => {
    ...
    await runMain();
    expect(coreMock.info).toHaveBeenCalledWith('no pull request numbers given: skip step');
  ...
  test('should log info and return early if body is empty and ignoreEmpty is true', async () => {
    ...
    vi.doMock('../src/config', async () => {
      const actual = await vi.importActual<typeof import('../src/config')>('../src/config');
      return { ...actual, getBody: MOCK_GET_BODY, ignoreEmpty: true, pullRequestNumber: 123, repo: {owner:'test',repo:'test'}, githubToken: 'token' };
    });
    await runMain();
    expect(coreMock.info).toHaveBeenCalledWith('no body given: skip step by ignoreEmpty');
    expect(MOCK_CREATE_COMMENT).not.toHaveBeenCalled();
    vi.doUnmock('../src/config');
  });
  ...
  test('should setFailed if body is empty, and not deleting or hiding', async () => {
    ...
    vi.doMock('../src/config', async () => {
      const actual = await vi.importActual<typeof import('../src/config')>('../src/config');
      return { ...actual, getBody: MOCK_GET_BODY, ignoreEmpty: false, deleteOldComment: false, hideOldComment: false, pullRequestNumber: 123, repo: {owner:'test',repo:'test'}, githubToken: 'token' };
    });
    await runMain();
    expect(coreMock.setFailed).toHaveBeenCalledWith('Either message or path input is required');
    vi.doUnmock('../src/config');
  });
});
...
describe('Input Validation Errors', () => { 
  const validationTestCases = [
    { name: 'deleteOldComment and recreate', props: { deleteOldComment: true, recreate: true }, expectedMsg: 'delete and recreate cannot be both set to true' }, 
    { name: 'onlyCreateComment and onlyUpdateComment', props: { onlyCreateComment: true, onlyUpdateComment: true }, expectedMsg: 'only_create and only_update cannot be both set to true' }, 
    { name: 'hideOldComment and hideAndRecreate', props: { hideOldComment: true, hideAndRecreate: true }, expectedMsg: 'hide and hide_and_recreate cannot be both set to true' }, 
  ];
  ...
  validationTestCases.forEach(tc => {
    test(`should setFailed if ${tc.name} are both true`, async () => {
      ...
      vi.doMock('../src/config', async () => {
        const actual = await vi.importActual<typeof import('../src/config')>('../src/config');
        return { ...actual, getBody: MOCK_GET_BODY, ...tc.props, pullRequestNumber: 123, repo: {owner:'test',repo:'test'}, githubToken: 'token' };
      });
      await runMain();
...
describe('Main Logic Scenarios', () => {
  describe('No Previous Comment', () => { 
    ...
    test('should not act if onlyUpdateComment is true', async () => {
      ...
      vi.doMock('../src/config', async () => {
        const actual = await vi.importActual<typeof import('../src/config')>('../src/config');
        return {...actual, getBody: MOCK_GET_BODY, onlyUpdateComment: true, pullRequestNumber: 123, repo: {owner: 'test', repo: 'test'}, githubToken: 'token' };
      });
      await runMain();
      expect(MOCK_CREATE_COMMENT).not.toHaveBeenCalled();
    ...
    test('should createComment if onlyUpdateComment is false', async () => {
      ...
      vi.doMock('../src/config', async () => {
         const actual = await vi.importActual<typeof import('../src/config')>('../src/config');
        return {...actual, getBody: MOCK_GET_BODY, onlyUpdateComment: false, pullRequestNumber: 123, repo: {owner: 'test', repo: 'test'}, githubToken: 'token' };
      });
      await runMain();
      expect(MOCK_CREATE_COMMENT).toHaveBeenCalled();
      expect(coreMock.setOutput).toHaveBeenCalledWith('previous_comment_id', undefined); 
      expect(coreMock.setOutput).toHaveBeenCalledWith('created_comment_id', 100);      
      vi.doUnmock('../src/config');
    });
  });
  ...
  describe('Previous Comment Exists', () => {
    ...
    test.skip('should not act if skipUnchanged is true and commentsEqual is true', async () => {
      ...
      expect(coreMock.info).toHaveBeenCalledWith('Comment is unchanged. Skipping.');
      expect(MOCK_UPDATE_COMMENT).not.toHaveBeenCalled();
      expect(MOCK_CREATE_COMMENT).not.toHaveBeenCalled();
    ...
    test('should deleteComment if deleteOldComment is true', async () => { 
      ...
      vi.doMock('../src/config', async () => { 
        const actual = await vi.importActual<typeof import('../src/config')>('../src/config');
        return {...actual, getBody: MOCK_GET_BODY, deleteOldComment: true, pullRequestNumber: 123, repo: {owner: 'test', repo: 'test'}, githubToken: 'token', header: testHeaderString };
      });
      await runMain();
      expect(MOCK_DELETE_COMMENT).toHaveBeenCalledWith(expect.any(Object), mockPrevComment.id);
      expect(coreMock.setOutput).toHaveBeenCalledWith('previous_comment_id', mockPrevComment.id);
      vi.doUnmock('../src/config');
    });
    test('should not act if onlyCreateComment is true', async () => { 
      ...
      vi.doMock('../src/config', async () => {
         const actual = await vi.importActual<typeof import('../src/config')>('../src/config');
        return {...actual, getBody: MOCK_GET_BODY, onlyCreateComment: true, pullRequestNumber: 123, repo: {owner: 'test', repo: 'test'}, githubToken: 'token', header: testHeaderString };
      });
      await runMain();
      expect(MOCK_CREATE_COMMENT).not.toHaveBeenCalled();
      expect(MOCK_UPDATE_COMMENT).not.toHaveBeenCalled();
    ...
    test('should minimizeComment if hideOldComment is true', async () => { 
      ...
      vi.doMock('../src/config', async () => {
         const actual = await vi.importActual<typeof import('../src/config')>('../src/config');
        return {...actual, getBody: MOCK_GET_BODY, hideOldComment: true, pullRequestNumber: 123, repo: {owner: 'test', repo: 'test'}, githubToken: 'token', hideClassify: 'OUTDATED', header: testHeaderString };
      });
      await runMain();
      expect(MOCK_MINIMIZE_COMMENT).toHaveBeenCalledWith(expect.any(Object), mockPrevComment.id, 'OUTDATED');
      expect(coreMock.setOutput).toHaveBeenCalledWith('previous_comment_id', mockPrevComment.id);
      vi.doUnmock('../src/config');
    });
    ...
    test('should delete then createComment if recreate is true', async () => { 
      ...
      vi.doMock('../src/config', async () => {
         const actual = await vi.importActual<typeof import('../src/config')>('../src/config');
        return {...actual, getBody: MOCK_GET_BODY, recreate: true, pullRequestNumber: 123, repo: {owner: 'test', repo: 'test'}, githubToken: 'token', header: testHeaderString };
      });
      await runMain();
      expect(MOCK_DELETE_COMMENT).toHaveBeenCalledWith(expect.any(Object), mockPrevComment.id);
      expect(MOCK_CREATE_COMMENT).toHaveBeenCalled();
    ...
    test('should minimize then createComment if hideAndRecreate is true', async () => { 
      ...
      vi.doMock('../src/config', async () => {
         const actual = await vi.importActual<typeof import('../src/config')>('../src/config');
        return {...actual, getBody: MOCK_GET_BODY, hideAndRecreate: true, pullRequestNumber: 123, repo: {owner: 'test', repo: 'test'}, githubToken: 'token', hideClassify: 'OUTDATED', header: testHeaderString };
      });
      await runMain();
      expect(MOCK_MINIMIZE_COMMENT).toHaveBeenCalledWith(expect.any(Object), mockPrevComment.id, 'OUTDATED');
      expect(MOCK_CREATE_COMMENT).toHaveBeenCalled();
    ...
    test('should updateComment by default', async () => { 
      ...
      vi.doMock('../src/config', async () => {
         const actual = await vi.importActual<typeof import('../src/config')>('../src/config');
        return {...actual, getBody: MOCK_GET_BODY, pullRequestNumber: 123, repo: {owner: 'test', repo: 'test'}, githubToken: 'token', header: testHeaderString };
      });
      await runMain();
...
describe('Error Handling', () => { 
  test('should setFailed if getBody throws', async () => {
    MOCK_GET_BODY.mockRejectedValue(new Error("GetBody Failed"));
     vi.doMock('../src/config', async () => {
        const actual = await vi.importActual<typeof import('../src/config')>('../src/config');
        return {...actual, getBody: MOCK_GET_BODY, deleteOldComment: true, pullRequestNumber: 123, repo: {owner:'test',repo:'test'}, githubToken: 'token' }; 
    });
    await runMain();
